...
export const storeSeismicData = async (data: SeismicData): Promise<void> => {
  try {
    ...
    console.error("Failed to store seismic data:", error);
  }
};

export const storeSupercarData = async (data: SupercarData): Promise<void> => {
  try {
    const client = getClient();
    const account = getAccount();
    const suggestedParams = await client.getTransactionParams().do();
    ...
    console.error("Failed to store supercar data:", error);
  }
};

export const storeBrandData = async (data: BrandData): Promise<void> => {
  try {
    const client = getClient();
    const account = getAccount();
    const suggestedParams = await client.getTransactionParams().do();
    ...
    console.error("Failed to store phone brand data:", error);
  }
};